...
- Phase A+B redesign foundation completed on 2026-07-14 (see CHANGELOG.txt):
  ...
  (`scripts/seed_fixture_library.py`), design-token system, responsive
  AppShell with consolidated navigation, and command-center Home at `/`.
- The library workspace moved from `/` to `/library`; all legacy routes
  redirect. New unified status model in `frontend/src/lib/status.ts`.
- Backend test suite: 862 passed (857 prior + 5 preflight). Frontend:
  typecheck + production build pass; 11 Vitest unit tests pass.
- Supervisor/preview runtime: backend served by `backend/server.py` shim on
  port 8001 (loads `backend/.env`), frontend Vite on port 3000 with `/api`
  proxy. Fixture library at `/app/fixture_library`.
